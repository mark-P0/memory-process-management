...
class Processor:
    ...
    queue: pd.DataFrame
    queue_cols: list[ProcessColumns] = list(ProcessColumns)[:3] + ["Suspended"]
    ...
    def dispatch(self, proc_name, runtime):
    ...
    def enqueue(self, proc, rem_time, queue_time, from_suspension):
        q = self.queue
        q.loc[len(q)] = (proc, rem_time, queue_time, from_suspension)
...
class Simulation:
    ...
    def __sim_arrivals(self):
        ...
        # elif priority_level < cpu.process["Priority"]:
        elif burst < cpu.process["Runtime"]:
            """
            ...
            ## Enqueue current process again
            curr_proc, curr_burst, *_ = cpu.process.values()
            # cpu.rr_enqueue(proc_name, runtime, is_suspended=True)
            cpu.enqueue(curr_proc, curr_burst, self.clock, True)
            ...
            cpu.enqueue(name_idx, burst, self.clock, False)
    ...
    def __sim_idle(self):
        ...
        if self.idle_label not in procs.index:
            procs.loc[self.idle_label] = -1, self.clock, self.clock, -1, -1, -1
    ...
    def __sim_proc_execute(self):
        ...
        if len(cpu.queue) > 0:
            ...
            proc_name, runtime, _, is_suspended = cpu.dequeue()
