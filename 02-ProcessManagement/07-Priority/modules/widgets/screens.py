...
class HomeScreen(Screen):
    ...
    def _create_table(self, rows=5):
        ...
        ## Add headers
        headers = ("Process", "Burst Time", "Arrival Time", "Priority No.")
        ...
        for row_ct in range(1, rows + 1):
            cells = (
                ...
                TableInput(),  # Priority level input
            )
    ...
    def _add_table_values(self):
        ...
        ## Test data
        key = "Lecture_Prio"
        data = core.LECTURE_EXAMPLES[key]

        Logger.info(f"Using data key `{key}`")
...
class LoadingScreen(Screen):
    ...
    def _prepare_data(self):
        ...
        # Process, Burst, Arrival, Priority, Start, Finish, Turnaround, Waiting, Color = core.ProcessColumns
        ...
        procs_props = sim.processes  # Does not need full table
    ...
    def _create_output_screens(self, base_name, app):
        ...
        screen_names = []
        Init, Idling, Release, Arrival, _, Suspend, _ = core.EventType
    ...
    def _create_summary_screens(self, app):
        # PC = core.ProcessColumns
        # _, Burst, Arrival, _, Start, Finish, Turnaround, Waiting, _ = PC
        ...
        name_map = {col: col.name for col in core.ProcessColumns}
        name_map[core.ProcessColumns.Turnaround] = "Trnard."
        name_map[core.ProcessColumns.Waiting] = "Wtng."
        char_tbl = char_tbl.rename(columns=name_map)

        ## Reduce columns
        keep_cols = ["Priority", "Process", "Arrival", "Burst", "Start", "Finish"]
        char_tbl = char_tbl[keep_cols].sort_values(by="Priority")
        ...
        idle_mask = events["Type"] == core.EventType.Idling
        events.at[idle_mask, "Process"] = ""  # Blank out idle period labels
    ...
    def _clean_jobs(self, valid_jobs):  # TODO: Refactor this? Hahaha
        ...
        col_map = {col.value: col for col in core.ProcessColumns}
        data[0] = [col_map[header] for header in data[0]]
        headers = data[0]
