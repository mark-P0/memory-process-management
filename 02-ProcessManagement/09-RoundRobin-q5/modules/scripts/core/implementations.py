...
class Simulation:
    ...
    def __init__(
        ...
        self.processor = Processor(quantum_time)
        self.processes = processes
    ...
    def __apply_scheduling(self, proc_slice):
        FCFS, SJF, Priority, SRTF, RR = SchedulingType

        # if self.scheduling == FCFS:
        #     proc_slice = proc_slice.sort_values(by="Arrival Time")
        #     proc_slice = proc_slice.sort_index()

        # elif self.scheduling == SJF:
        #     proc_slice = proc_slice.sort_values(by="Burst Time")
        #     proc_slice = proc_slice.sort_index()
        ...
        if self.scheduling == RR:
            ...
            raise NotImplementedError(f"Type `{self.scheduling}` not implemented.")
